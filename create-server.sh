...
create_server() {
    ...
    PRIVATE_IP=$(aws ec2 run-instances --security-group-ids $SGID --image-id  $AMI_ID --instance-type t3.micro --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=${COMPONENT}-${ENV}}]" | jq '.Instances[].PrivateIpAddress' | sed -e 's/"//g')
    ...
    aws route53 change-resource-record-sets --hosted-zone-id Z096855931OR9MBP19PUW --change-batch file:///tmp/record.json | jq 
}
